...
//    int orange_1;
    ...
    View.OnClickListener myClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            detailIntent.putExtra("scoresJSON", scoresJSON.toString());
    ...
    protected void displayData (JSONObject scoresJSON){
        this.scoresJSON = scoresJSON;
        //Log.d("MCCArchers", "loading into Activity: "+scoresJSON.toString());
        try {
            endTotal = 0;
            grandTotal = 0;
            int tempArrow = 0;
            ...
            try {
                ...
                endsArr = scoresJSON.getJSONArray("data");
            } catch (Throwable t) {
                Log.e("MCCArchers", "error getting data from scoresJSON (RoundScores.displayData");
                t.printStackTrace();
            }
            if (!scoresJSON.has("status")) {
                JSONObject scoresStatusJSON = new JSONObject().put("currentEnd", currentEnd);
                scoresStatusJSON.put("currentArrow", currentArrow);
                scoresJSON.put("status", scoresStatusJSON);
            };
    ...
    protected void loadData() {
        ...
        //TODO : BUSY : change this so that it uses the SQLLite database, rather than the Firebase

        SQLiteLocal db = new SQLiteLocal(this);
        LocalRound thisRound = db.getLocalRound(roundID);
        displayData(thisRound.getScoreJSON());
        //here - this should now work if the data in the SQLiteLocal is correct
