    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myAppState.CDS.myConnectionListener = new ClubFirebase.OnConnectionListener() {
            ...
            public void onConnectionUpdated(Boolean SyncOn, Boolean Network, Boolean CDSConnected, Boolean Authenticated, Boolean Linked, String ErrorMessage) {
                ...
                if (Network) {IVNetwork.setImageResource(R.drawable.ic_action_ok);} else {IVNetwork.setImageResource(R.drawable.ic_action_nok);}
                if (CDSConnected == null) {IVConnected.setImageResource(R.drawable.ic_action_wait);} else if (CDSConnected) {IVConnected.setImageResource(R.drawable.ic_action_ok);} else {IVConnected.setImageResource(R.drawable.ic_action_nok);}
                if (Authenticated == null) {IVAuthenticated.setImageResource(R.drawable.ic_action_wait);}else if (Authenticated) {IVAuthenticated.setImageResource(R.drawable.ic_action_ok);} else {IVAuthenticated.setImageResource(R.drawable.ic_action_nok);}
                if (Linked == null) {IVLinked.setImageResource(R.drawable.ic_action_wait);}else if (Linked) {IVLinked.setImageResource(R.drawable.ic_action_ok);} else {IVLinked.setImageResource(R.drawable.ic_action_nok);}
                linked = Linked;
        ...
        //ToDo ( RELEASE > 2) : What happens if not OK?
